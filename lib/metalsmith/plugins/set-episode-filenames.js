...
const debug = require('debug')('set-episode-filenames')  // DEBUG=set-episode-filenames
const paths = require('../../helpers/file-paths') // helper to get build system paths

/**
 * Set Episode Filenames (Metalsmith plugin)
 *
 * Create composite path names for episodes based on number and title
 *
 ...
 *
 */
function setEpisodeFilenamesPlugin (opts) {
  const defaults = {
  ...
  return function setEpisodeFilenames (files, metalsmith, done) {
...
module.exports = setEpisodeFilenamesPlugin
// require this plugin in ./tasks/metalsmith using:
// const setEpisodeFilenames = require(paths.lib('metalsmith/plugins/set-episode-filenames.js'))
